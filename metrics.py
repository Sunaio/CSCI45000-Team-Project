...
class Metrics:
    ...
    def run(self) -> Dict[str, float]:
        ...
        format_results = OrderedDict([
            ("name", self.mod.model_dict.get("name") or "bert-base-uncased"),
            ("category", "MODEL"),
            ("net_score", float(net_res["net_score"])),
            ("net_score_latency", float(net_res["net_score_latency"])),
            ("ramp_up_time", float(ramp_res["ramp_up_time"])),
            ("ramp_up_time_latency", float(ramp_res["ramp_up_time_latency"])),
            ("bus_factor", float(bus_res["bus_factor"])),
            ("bus_factor_latency", float(bus_res["bus_factor_latency"])),
            ("performance_claims", float(perf_res["performance_claims"])),
            ("performance_claims_latency", float(perf_res["performance_claims_latency"])),
            ("license", float(license_res["license"])),
            ("license_latency", float(license_res["license_latency"])),
            ("size_score", size_res["size_score"]),
            ("size_score_latency", float(size_res["size_score_latency"])),
            ("dataset_and_code_score", float(ds_code_res["dataset_and_code_score"])),
            ("dataset_and_code_score_latency", float(ds_code_res["dataset_and_code_score_latency"])),
            ("dataset_quality", float(ds_quality_res["dataset_quality"])),
            ("dataset_quality_latency", float(ds_quality_res["dataset_quality_latency"])),
            ("code_quality", float(code_quality_res["code_quality"])),
            ("code_quality_latency", float(code_quality_res["code_quality_latency"]))
            ]
        )
